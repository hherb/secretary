...
- `cargo test --release --workspace --no-fail-fast` → **759 / 0 / 10** (750 baseline from `a46c829` + 9 = 1 new sync_merge integration test + 4 copies of `sync_helpers::helper_tests::rewrite_block_and_update_manifest_round_trips_through_open_vault` + 4 copies of `sync_helpers::helper_tests::rewrite_block_and_update_manifest_panics_on_shared_nonce` — both helper tests run once per integration-test target that declares `mod sync_helpers;`). The `_panics_on_shared_nonce` test landed in the PR #96 review-fix commit (asserts the new `assert_ne!(block_seed, manifest_nonce)` guard fires under `cargo test --release`, where `debug_assert_ne!` would have been compiled out).
...
- [ ] `cargo test --release --workspace --no-fail-fast` → 759+ / 0 / 10 (750 baseline at `a46c829` + ≥9 new tests across Tasks 9-16; we're at 759 after Task 9 + review-fix = 750 + 1 (Task 9 integration) + 4 (helper round-trip test re-run across 4 sync_helpers consumers) + 4 (helper panics-on-shared-nonce test re-run across same))
- [ ] `cargo clippy --release --workspace --tests -- -D warnings` → clean
- [ ] `cargo fmt --all -- --check` → clean
- [ ] `uv run core/tests/python/conformance.py` → PASS
...
# Baseline gauntlet (expect 759 / 0 / 10 on this branch before Task 10 starts):
...
- **Workspace tests on `feature/c1-1b-sync-merge`:** 759 passed + 10 ignored (750 baseline from `a46c829` + 9 = 1 new sync_merge integration test + 4 helper-round-trip re-runs + 4 helper-panics-on-shared-nonce re-runs across the four `sync_helpers`-consuming integration binaries). Clippy + fmt + Python conformance + spec-citation freshness all clean.
