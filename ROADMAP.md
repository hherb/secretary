...
- **Three internal A.7 hardening passes (2026-05-02)** all ✅ closed:
  ...
  - **Memory-hygiene audit**: audit memo at [`docs/manual/contributors/memory-hygiene-audit-internal.md`](docs/manual/contributors/memory-hygiene-audit-internal.md). Wrapper discipline (`Sensitive<T>` / `SecretBytes`) and composite-type drop ordering verified clean; **twelve stack-residue gaps fixed** (sister sites to already-disciplined ones — the established `var.zeroize()` after `Sensitive::new(var)` pattern). One large gap deferred to v2 design discussion: `RecordFieldValue::{Text, Bytes}` (passwords / secret notes) is the only secret-bearing data in the codebase NOT zeroize-on-drop; fix requires non-trivial API changes that ripple through ~100 test sites and the Python conformance verifier.
...
- **Memory hygiene audit** ✅ — audit memo at [`docs/manual/contributors/memory-hygiene-audit-internal.md`](docs/manual/contributors/memory-hygiene-audit-internal.md). Wrapper discipline (`Sensitive<T>` / `SecretBytes`) sound; composite-type drop ordering verified clean. **Twelve stack-residue gaps fixed** in commit `6054185` (one-line `var.zeroize()` after `Sensitive::new(var)` — sister sites to already-disciplined ones). One large gap deferred to v2: `RecordFieldValue::{Text, Bytes}` is the only secret-bearing data in the codebase NOT zeroize-on-drop; fix requires non-trivial `SecretString`-typing API change.
