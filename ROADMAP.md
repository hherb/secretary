...
- **B.4b** — Block read: `read_block(uuid)` exposed — extends `OpenVaultManifest` with the first mutation-free block-access operation. Record / RecordField / RecordFieldValue projection to Python `dataclass` / Swift `struct` / Kotlin `data class` with zeroize discipline (main open design question).
