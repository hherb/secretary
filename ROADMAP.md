...
## Sub-project B — FFI bindings 🚧 (B.1 Python complete; B.1.1 Swift complete; B.1.1.1 Kotlin complete; B.2 vault unlock complete; B.3+ pending)
...
- **B.2 (Vault unlock through FFI)** ✅ — `open_with_password` exposed across all three foreign languages through the new shared [`secretary-ffi-bridge`](ffi/secretary-ffi-bridge/) crate. First fallible operation (`PyResult` ergonomics, uniffi `[Throws]` exception marshalling) and first secret-bearing type across the FFI boundary (zeroize discipline through opaque-handle close). Bridge crate thins core's 7-variant `UnlockError` to a user-actionable 3-variant surface (`WrongPasswordOrCorrupt` / `VaultMismatch` / `CorruptVault { message }`); deliberately conflates wrong-password and corruption per `docs/threat-model.md` §13's anti-oracle property. RAII lifecycle in each foreign language: Python `with`, Swift `defer { wipe() }`, Kotlin `.use { }` via uniffi 0.31's auto-generated `AutoCloseable`. uniffi-codegen-driven renames (`close → wipe`, `message → detail`) are projection-only — the bridge crate's API is unchanged. Sibling `golden_vault_002/` fixture supports cross-vault `VaultMismatch` testing. 7 new pytests + 4 new Swift asserts + 4 new Kotlin asserts; 20 new bridge-crate Rust tests. Spec at [docs/superpowers/specs/2026-05-04-ffi-b2-vault-unlock-design.md](docs/superpowers/specs/2026-05-04-ffi-b2-vault-unlock-design.md); plan at [docs/superpowers/plans/2026-05-04-ffi-b2-vault-unlock.md](docs/superpowers/plans/2026-05-04-ffi-b2-vault-unlock.md); bridge crate README at [ffi/secretary-ffi-bridge/README.md](ffi/secretary-ffi-bridge/README.md).
