...
## Sub-project B — FFI bindings 🚧 (B.1 Python complete; B.1.1 Swift complete; B.1.1.1 Kotlin complete; B.2 password unlock complete; B.3a recovery unlock complete; B.3b create_vault pending)
...
- **B.3b** — `create_vault` and the output-direction mnemonic-marshalling case (the "how does `Sensitive<T>` materialize on the foreign side?" question deferred from B.2's non-goals). Re-opens RNG-seam and KDF-params-ergonomics decisions at the FFI boundary.
