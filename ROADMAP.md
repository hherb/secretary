...
Sub-project A is feature-complete for v1; Phase A.7 (hardening + external audit) is in progress. Phase A.6 / PR-C landed the CRDT merge primitives (`core/src/vault/conflict.rs`: `clock_relation`, `merge_vector_clocks`, `merge_record`, `merge_block`) plus a record-level `tombstoned_at_ms` "death-clock" that closes the three-way-merge associativity gap that naive tombstone-on-tie semantics leave open. PR #9 added the polishing pass: a bidirectional defensive clamp on `tombstoned_at_ms`, tag-canonicalisation on the LWW-clone path, and a clean-room Python `py_merge_unknown_map` for the record-level `unknown` map — extending the §15 cross-language KAT to **eleven** vectors (`core/tests/data/conflict_kat.json`) covering every `ClockRelation` branch, the death-clock staleness filter, the §11.3 identity-metadata override, and the record-level `unknown` collision + tombstone-wins paths. Definition-of-Done item #3 in the design anchor (commutativity + associativity + idempotence under random sequences of edits) is satisfied across the *full* record domain — arbitrary tombstones, arbitrary resurrection sequences, arbitrary record-level `unknown` keys — proven by four `proptest` properties (the original three plus a new well-formedness Property L) at default 256 cases each in `core/tests/proptest.rs`. The §11.3 identity-metadata override (where the merged record's `tags` / `record_type` / record-level `unknown` come wholesale from the tombstoning side) completes the design — readers of the spec can implement an interoperable client from `docs/crypto-design.md` §11 + `docs/vault-format.md` §6.3 alone, and Python's clean-room `py_merge_record` + `py_merge_unknown_map` prove that contract every CI run.

In parallel, Phase A.7 has begun: PR #8 landed a coverage-guided fuzz harness (`core/fuzz/`, `cargo-fuzz`) covering all six wire-format decoders (`block_file`, `bundle_file`, `contact_card`, `manifest_file`, `record`, `vault_toml`), with a single-file NiceGUI dashboard (`core/fuzz/monitor.py`) for running and watching campaigns and a [`differential-replay-protocol`](docs/manual/contributors/differential-replay-protocol.md) for cross-language replay through the Python conformance script. The PR #11 follow-up triaged the harness's first six artifacts — they could not be reproduced against current main (libfuzzer false positives from accumulated long-campaign state), but the inputs were promoted as committed regression tests under [`core/tests/data/fuzz_regressions/`](core/tests/data/fuzz_regressions/), and a real peer-supplied DoS surface caught while reading the contact-card decoder (`display_name` was unbounded variable-length CBOR text on a sync-reachable path) was capped at 4 KiB on parse, with the cap enforced symmetrically on encode and decode. PR #12 then made the NiceGUI monitor a useful diagnostic surface for live campaigns: per-card cov/ft/corp/exec-s/rss readout, elapsed timer + runs/cap progress, reactive Status badge with per-status colour, and a global findings counter above the grid. PR #10 added a thirteen-chapter [cryptography primer](docs/manual/primer/cryptography/index.md) for users and contributors who want the conceptual background without reading the normative spec.

```
[================================================================] Sub-project A — Rust core (feature-complete; A.7 hardening in progress)
[                                                                ] Sub-project B — FFI bindings
[                                                                ] Sub-project C — Sync orchestration
[                                                                ] Sub-project D — Platform UIs
```

430+ tests pass under `cargo test --release --workspace`. Clippy is clean with `-D warnings`. `#![forbid(unsafe_code)]` is crate-wide. Every cryptographic primitive is pinned against published KATs; the CRDT merge layer is additionally proven on the full record domain (including arbitrary record-level `unknown`) by four `proptest` properties. Both `golden_vault_001/` (full crypto) and `conflict_kat.json` (merge semantics, eleven vectors) are verified end-to-end by the Rust suite and by the stdlib-only Python conformance script.
...
### Phase A.7 — Hardening + audit prep 🚧 (in progress)

- **Fuzz harness** ✅ landed in PR #8: `core/fuzz/` with `cargo-fuzz` on a path-scoped nightly toolchain, six targets (`block_file`, `bundle_file`, `contact_card`, `manifest_file`, `record`, `vault_toml`) seeded from §15 KAT fixtures, plus a single-file NiceGUI dashboard ([`core/fuzz/monitor.py`](core/fuzz/monitor.py)) for running and watching campaigns. Cross-language differential-replay protocol contracted in [`docs/manual/contributors/differential-replay-protocol.md`](docs/manual/contributors/differential-replay-protocol.md). PR #11 closed out the harness's first six artifacts (libfuzzer false positives, promoted as regression tests under [`core/tests/data/fuzz_regressions/`](core/tests/data/fuzz_regressions/) with a real DoS-bound on the contact-card `display_name` field as defense-in-depth, enforced symmetrically on encode and decode). PR #12 then plumbed the existing pulse stream through to the dashboard: per-card cov/ft/corp/exec-s/rss readout, elapsed timer + runs/cap progress, reactive status badge with per-status Quasar colour, and a global findings counter above the card grid — pure formatters fully unit-tested in [`core/fuzz/test_monitor.py`](core/fuzz/test_monitor.py).
- **Independent cryptographic review** (paid, external). Spec docs are stable enough to send out; reviewer with FIPS 203 / FIPS 204 implementer experience and AAD/signed-range eyes especially valuable.
- **Side-channel review**. Constant-time critical paths — verify-then-decap flows, `Fingerprint` comparison and recipient-table lookup, Argon2id comparison sites in unlock paths.
- **Memory hygiene audit** — `zeroize` coverage on every secret type, `secrecy::Secret` typestate where load-bearing, drop ordering in `IdentityBundle` / `BlockPlaintext` / `Identity`, especially across `?` propagation sites.
- **Documentation pass**: `docs/threat-model.md` updated to reference the as-implemented surface; `docs/vault-format.md` clarifications surfaced during implementation (two known tickets — §6.2 wrap_ct/wrap_tag wire-form, §6.1 `sig_pq_len = u16, 3309 (suite v1)` annotation). Polish dribbles tracked in [docs/TODO_FINAL_POLISHING.md](docs/TODO_FINAL_POLISHING.md).
- **User-facing primer** (PR #10) ✅ landed: a thirteen-chapter [cryptography primer](docs/manual/primer/cryptography/index.md) for users and contributors. Bonus material — not strictly Phase A.7 — but valuable contributor onboarding now that the spec has stabilised.
