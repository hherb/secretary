...
[============                                                    ] Sub-project B — FFI bindings (B.1 Python ✅; B.1.1 Swift ✅; Kotlin runner → B.1.1.1; B.2+ vault crypto pending)
...
## Sub-project B — FFI bindings 🚧 (B.1 Python complete; B.1.1 Swift complete; Kotlin runner + crypto exposure pending)
...
- **uniffi** ([ffi/secretary-ffi-uniffi](ffi/secretary-ffi-uniffi/)): Swift bindings for iOS, Kotlin bindings for Android — same UDL, two outputs. As of B.1.1: macOS-host Swift smoke runner ✅; Kotlin runner deferred to B.1.1.1.
...
- **B.1.1.1 (Kotlin smoke runner)** ⏳ — bindgen already emits Kotlin bindings (`--language kotlin`); the gap is a JVM/JNI runner harness or Android-emulator integration so the Kotlin output is exercised the way Swift is.
