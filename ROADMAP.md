...
## Sub-project B — FFI bindings 🚧 (B.1 Python complete; B.1.1 Swift complete; B.1.1.1 Kotlin complete; B.2 password unlock complete; B.3a recovery unlock complete; B.3b create_vault complete; B.4a open_vault complete)
...
- **B.4b** — Block read: `read_block(&identity, &manifest, &[u8; 16])` exposed as a free function on the bridge — first mutation-free block-access operation. Hybrid Record projection (resolved): value-type metadata (record_uuid, record_type, tags, timestamps, tombstone) + opaque `FieldHandle` for secret payloads with explicit `expose_text()` / `expose_bytes()` boundary. Three new opaque handles (`BlockReadOutput`, `Record`, `FieldHandle`) all share the `Mutex<Option<...>>` + cascading `wipe()` pattern; `Record`/`FieldHandle` add `Arc` for clone-safe sharing. `FfiVaultError` grows 6 → 7 variants (+ `BlockNotFound { uuid_hex }`); decrypt + file-missing failures fold into `CorruptVault` per anti-conflation discipline. `OpenVaultManifestInner` extended internally with `vault_folder: PathBuf` for path resolution (no public B.4a accessor change). Trash deferred to Sub-project C; multi-author block reading deferred to B.4d. Defense-in-depth fuzz target extension on `record::decode` Text-field UTF-8 validity. Spec at [docs/superpowers/specs/2026-05-09-ffi-b4b-read-block-design.md](docs/superpowers/specs/2026-05-09-ffi-b4b-read-block-design.md); implementation plan + code pending.
- **B.4c** — Block save: `save_block` exposed — first mutation path, atomic write ordering per §9.
- **B.4d** — Block share: `share_block` exposed — ContactCard surface first crosses the FFI.
- **B.5** — Conformance: same `golden_vault_001/` test runs in Python and Rust and Swift and Kotlin and produces bit-identical results.
