...
| Sync orchestration (file watching, cloud-folder integration, conflict-detection scheduling — headless, exposed via FFI) | ⏳ Sub-project C (C.1 phase 1 ✅ — pure-Rust `core::sync::sync_once` classifies one vault folder against caller-persisted `SyncState` into `NothingToDo` / `AppliedAutomatically` / `ConcurrentDetected` / `RollbackRejected`. C.1.1a ✅ — `ConcurrentDetected` now carries an authenticated `VaultBundle` of sibling conflict-copies (Dropbox / iCloud / Syncthing naming conventions detected by prefix, security gated by §1a-D4's five MUST authentication rules: decode + Ed25519 ∧ ML-DSA-65 hybrid signature + canonical vault_uuid + canonical owner fingerprint + AEAD-decrypts under the unlocked IBK). C.1.1b ✅ — three-step merge API (`sync_once → prepare_merge → commit_with_decisions`) over the `VaultBundle`; record-level tombstone vetoes surface to the caller as a `Vec<RecordTombstoneVeto>` and resolve via `VetoDecision::{KeepLocal, AcceptTombstone}` bijectively. Block-first / manifest-last atomic write order plus eager `verify_block_fingerprints` on `open_vault` make partial commits recoverable via idempotent re-run of the three-step flow — the CRDT idempotence proof carries the convergence guarantee, no orchestrator-side journaling needed.) |
