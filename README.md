...
| FFI bindings (vault unlock exposed) | ✅ Sub-project B.2 (`open_with_password` through PyO3 + uniffi via shared `secretary-ffi-bridge` crate; thinned 3-variant error type; explicit close + RAII lifecycle) |
| FFI bindings (block save / share / open) | ⏳ Sub-project B.3+ |
...
477 tests pass + 9 ignored under `cargo test --release --workspace` (448 from `secretary-core` + 20 from the new `secretary-ffi-bridge` crate that holds the FFI-friendly facade shared by both binding flavors + 3 from `secretary-ffi-py` + 6 from `secretary-ffi-uniffi`); clippy clean with `-D warnings`; `#![forbid(unsafe_code)]` crate-wide for `core/` *and* for `secretary-ffi-bridge/` (the bridge is pure-safe Rust). Both binding-flavor crates carry a localized `unsafe_code = "deny"` carve-out — `secretary-ffi-py/` for PyO3 macro expansions and `secretary-ffi-uniffi/` for `uniffi::include_scaffolding!()`'s `unsafe extern "C"` blocks — per the "FFI as isolated reviewed boundary" principle (`forbid` is non-overridable by inner `#[allow]`, so each crate downgrades to `deny` and carries a single crate-level `#![allow]` with rationale). The Python pytest layer covers 10 tests (3 B.1 smoke + 7 B.2 unlock); the Swift smoke runner covers 7 asserts (3 B.1.1 smoke + 4 B.2 unlock); the Kotlin smoke runner covers 7 asserts (3 B.1.1.1 smoke + 4 B.2 unlock). The repository tracks every cryptographic decision in `docs/adr/` and pins every primitive against published KATs (NIST FIPS 203 / FIPS 204, RFC 8032 / 7748, RFC 5869, RFC 9106, BIP-39 Trezor canonical vectors). The CRDT merge layer is additionally proven commutative, associative, idempotent, and well-formed over the full record domain — including arbitrary tombstone-and-resurrection histories and arbitrary record-level `unknown` keys — by four `proptest` properties at default (~256 cases each) under `core/tests/proptest.rs`'s PR-C / PR #9 section.
