...
| FFI bindings (block read / save / share) | ⏳ Sub-project B.4b/c/d |
...
518 tests pass + 9 ignored under `cargo test --release --workspace` (448 from `secretary-core` + 54 from the `secretary-ffi-bridge` crate that holds the FFI-friendly facade shared by both binding flavors + 3 from `secretary-ffi-py` + 13 from `secretary-ffi-uniffi`); clippy clean with `-D warnings`; `#![forbid(unsafe_code)]` crate-wide for `core/` *and* for `secretary-ffi-bridge/` (the bridge is pure-safe Rust). Both binding-flavor crates carry a localized `unsafe_code = "deny"` carve-out — `secretary-ffi-py/` for PyO3 macro expansions and `secretary-ffi-uniffi/` for `uniffi::include_scaffolding!()`'s `unsafe extern "C"` blocks — per the "FFI as isolated reviewed boundary" principle (`forbid` is non-overridable by inner `#[allow]`, so each crate downgrades to `deny` and carries a single crate-level `#![allow]` with rationale). The Python pytest layer covers 29 tests (3 B.1 smoke + 7 B.2 password-unlock + 6 B.3a recovery-unlock + 6 B.3b vault-creation + 7 B.4a folder-open); the Swift smoke runner covers 18 asserts (3 B.1.1 smoke + 5 B.2 password-unlock + 4 B.3a recovery-unlock + 3 B.3b vault-creation + 3 B.4a folder-open); the Kotlin smoke runner covers 19 asserts (3 B.1.1.1 smoke + 5 B.2 password-unlock + 4 B.3a recovery-unlock + 3 B.3b vault-creation + 4 B.4a folder-open). The repository tracks every cryptographic decision in `docs/adr/` and pins every primitive against published KATs (NIST FIPS 203 / FIPS 204, RFC 8032 / 7748, RFC 5869, RFC 9106, BIP-39 Trezor canonical vectors). The CRDT merge layer is additionally proven commutative, associative, idempotent, and well-formed over the full record domain — including arbitrary tombstone-and-resurrection histories and arbitrary record-level `unknown` keys — by four `proptest` properties at default (~256 cases each) under `core/tests/proptest.rs`'s PR-C / PR #9 section.
