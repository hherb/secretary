...
> **Status: pre-alpha, Sub-project A is feature-complete for v1; Phase A.7 hardening + external audit in progress; Sub-project B is now in flight with B.1 / B.1.1 / B.1.1.1 / B.2 / B.3a / B.3b / B.4a / B.4b / B.4c / B.4d / B.5 all shipped.** Architecture and cryptographic design are complete and frozen for v1. The Rust core covers cryptographic primitives, identity, vault unlock, the block file format, the manifest layer with atomic I/O, the high-level orchestrators (`create_vault`, `open_vault`, `save_block`, `share_block`), and the vector-clock CRDT merge primitives (`merge_record` and `merge_block` with a record-level death-clock that makes the merge associative across arbitrary tombstone histories, plus PR #9's bidirectional defensive clamp, tag canonicalisation, and clean-room Python `py_merge_unknown_map` for record-level `unknown` cross-language). 552 tests pass + 9 ignored, including NIST KATs, four property-based CRDT proofs (commutativity, associativity, idempotence, well-formedness under arbitrary inputs) on the full record domain at proptest defaults, and a stdlib-only Python conformance script that does full hybrid-decap + AEAD-decrypt + hybrid-verify against the `golden_vault_001` fixture and replays eleven merge KATs cross-language. Phase A.7 is underway: a coverage-guided `cargo-fuzz` harness over all six wire-format decoders landed in PR #8, its first batch of artifacts was triaged and closed in PR #11 (libfuzzer false positives promoted as regression tests under [`core/tests/data/fuzz_regressions/`](core/tests/data/fuzz_regressions/) plus a real peer-supplied DoS-bound on the contact-card `display_name` field, capped symmetrically on encode and decode), and PR #12 surfaced live telemetry on the NiceGUI monitor (status badge per Status, `cov / ft / corp / exec-s / rss` per card, elapsed timer + runs/cap progress, global findings tally). PR #10 added a thirteen-chapter [cryptography primer](docs/manual/primer/cryptography/index.md) for users and contributors. **Sub-project B is now in flight**: B.1 (Python FFI binding boilerplate) is complete — PyO3 + maturin wired up, two trivial round-trip functions (`add`, `version`) exposed. **B.1.1 (Swift via uniffi)** and **B.1.1.1 (Kotlin smoke runner)** complete the foreign-language pair from the same UDL → cdylib pipeline, verified by macOS-host swiftc and JVM-host JNA smoke runners. **B.2 (vault unlock through FFI)** exposed `open_with_password` through both PyO3 and uniffi via a new shared `secretary-ffi-bridge` crate (single source of FFI code truth, no drift between Python and Swift/Kotlin). **B.3a (recovery-phrase unlock through FFI)** completes the unlock surface: `open_with_recovery` is now exposed through both binding flavors via the same bridge crate; `FfiUnlockError` grew from 3 → 5 variants (`WrongMnemonicOrCorrupt`, `InvalidMnemonic { detail }`) preserving the §13 anti-oracle conflation across both unlock paths; mnemonic input crosses as caller-zeroizable bytes parallel to B.2's password input; 6 new pytests + 4 new Swift asserts + 4 new Kotlin asserts pin every error path from each foreign language; the existing `golden_vault_001/` and `golden_vault_002/` fixtures were re-used unchanged on disk via a new `recovery_mnemonic_phrase` field in their inputs JSON, kept honest by a `bip39::Mnemonic::from_entropy` drift-detection assertion. **B.3b (vault creation through FFI)** completes the creation surface: `create_vault` is now exposed through both binding flavors via the same bridge crate, returning a four-field `CreateVaultOutput` (`vault_toml_bytes`, `identity_bundle_bytes`, live `UnlockedIdentity` handle, one-shot `MnemonicOutput`); the bridge instantiates `OsRng` + `Argon2idParams::V1_DEFAULT` internally so foreign callers cannot tune those knobs; the recovery mnemonic exits the `Sensitive<T>` boundary as caller-owned bytes via `take_phrase()` (one-shot, second call returns `None`/`null`/`None` per language); `CorruptVault`'s Display text was made path-neutral so the variant reads correctly on both open and create paths. **B.4a (folder-based vault open through FFI)** establishes the folder-IO model: `open_vault_with_password` and `open_vault_with_recovery` are now exposed through both binding flavors via the same bridge crate, reading `vault.toml`, `identity.bundle.enc`, `manifest.cbor.enc`, and the owner contact card from a named folder — the first folder-IO entry points on the FFI surface, setting the IO model that B.4b/c/d will inherit; a new 6-variant `FfiVaultError` mirrors the 5 unlock-class variants byte-identically and adds `FolderInvalid { detail }`; a new `OpenVaultManifest` opaque handle holds IBK + manifest + envelope + verified owner card internally and exposes a read-only block list (`vault_uuid`, `owner_user_uuid`, `block_count`, `block_summaries`, `find_block`) via `BlockSummary` value types (5 plaintext-in-the-manifest fields); `local_highest_clock` is always `None` in B.4a — rollback detection is deferred to Sub-project C; test counts grew to 518 cargo + 9 ignored, 29 pytest, 18 Swift PASS, 19 Kotlin PASS. **B.4b (block read through FFI)** completes the first mutation-free block-access path: `read_block(&identity, &manifest, &[u8; 16])` is now exposed as a free function on the bridge, projected through PyO3 + uniffi with three new opaque handles (`BlockReadOutput`, `Record`, `FieldHandle`) under a hybrid Record projection — non-secret metadata is value-typed; secret payload is opaque-handle with explicit `expose_text()` / `expose_bytes()` boundary. `FfiVaultError` grows 6 → 7 variants (+ `BlockNotFound { uuid_hex }`); decrypt + file-missing failures fold into `CorruptVault` per anti-conflation discipline. `OpenVaultManifestInner` is extended internally with `vault_folder: PathBuf` (no public B.4a surface change) so `read_block` resolves `blocks/<uuid>.cbor.enc` directly. v1 single-author block reading only — multi-author flow deferred to B.4d. Defense-in-depth UTF-8-validity assertion added to the `record` fuzz target. Test counts grew to **552 cargo + 9 ignored, 40 pytest, 22 Swift PASS, 23 Kotlin PASS** (post-PR-#31 review fixes); bridge crate Rust tests grew from 60 → 83 (the new `record/` directory module split into 5 sub-files per project policy + `tests/read_block.rs` integration suite + 2 from `ReaderSecretKeysError` typed-error refactor in review fixes); uniffi crate tests grew from 15 → 18 (split across `errors.rs` + `wrappers/{identity,vault,block}.rs` + `namespace.rs` + `lib.rs` in the post-merge cleanup pass — see the [chore/b4b-deferred-cleanup branch](https://github.com/hherb/secretary)). **B.4c (block save through FFI)** completes the first mutation path: `save_block(&identity, &manifest, BlockInput, device_uuid, now_ms)` is exposed as a free function on the bridge, projected through PyO3 + uniffi with a foreign-side input shape (`BlockInput` / `RecordInput` / `FieldInput` / `FieldInputValue`) that wraps text + bytes payloads in `secretary_core`'s zeroize-on-drop `SecretString` / `SecretBytes` carriers. `FfiVaultError` grows 7 → 9 variants — `SaveCryptoFailure { detail }` for in-memory failures on already-validated inputs (clock overflow, post-unlock memory corruption, encoder failure, signature primitive failure), and `InvalidArgument` is now raised from any wrong-length UUID at the input boundary. The save orchestrator builds a temporary `OpenVault` from clones (manifest, manifest_file, IBK, identity bundle), runs `secretary_core::vault::save_block`, and on `Ok` writes the mutated manifest back into the bridge handle atomically; on `Err` the bridge in-memory state is byte-identical to pre-call (a 7-test `tests/save_block.rs` integration suite pins this invariant including a `cfg(unix)` chmod-readonly path that asserts the manifest is unchanged after a failed save). Manifest re-sign cost (~5 ms Ed25519 + ML-DSA-65 per save) is acceptable for v1 single-threaded UIs. v1 single-author only — multi-recipient flow deferred to B.4d. Test counts grew to **574 cargo + 9 ignored, 50 pytest, 26 Swift PASS, 27 Kotlin PASS**. What remains in Sub-project A is independent cryptographic review and the side-channel / memory-hygiene audits; subsequent FFI work (B.4d for block share), the headless sync-orchestration layer (Sub-project C), and platform UIs (Sub-project D) come after that. There is no usable application yet. See [docs/](docs/) for the design and [ROADMAP.md](ROADMAP.md) for the phase plan.
...
642 tests pass + 9 ignored under `cargo test --release --workspace` (B.5 added 39 net tests: +5 bridge error-mapping pin tests, +4 uniffi error-mapping pin tests, +11 core integration tests in `core/tests/trash_restore.rs`, +1 proptest in `core/tests/trash_restore_proptest.rs` at 16 cases, +5 bridge unit tests for the `trash/` + `restore/` modules' mappers, +6 bridge integration tests across `tests/trash_block.rs` and `tests/restore_block.rs`, and +3 review-fix tests in PR #47 — orphan-trash-file rejection, contact-card self-verify on contacts/ scan, and non-canonical trash-suffix skipping); clippy clean with `-D warnings`; `#![forbid(unsafe_code)]` crate-wide for `core/` *and* for `secretary-ffi-bridge/` (the bridge is pure-safe Rust). Both binding-flavor crates carry a localized `unsafe_code = "deny"` carve-out — `secretary-ffi-py/` for PyO3 macro expansions and `secretary-ffi-uniffi/` for `uniffi::include_scaffolding!()`'s `unsafe extern "C"` blocks — per the "FFI as isolated reviewed boundary" principle (`forbid` is non-overridable by inner `#[allow]`, so each crate downgrades to `deny` and carries a single crate-level `#![allow]` with rationale). The Python pytest layer covers 68 tests (3 B.1 smoke + 7 B.2 password-unlock + 6 B.3a recovery-unlock + 6 B.3b vault-creation + 7 B.4a folder-open + 11 B.4b read-block + 10 B.4c save-block + 7 B.4d share-block + 11 B.5 trash + restore); the Swift smoke runner covers 34 asserts (3 B.1.1 smoke + 5 B.2 password-unlock + 4 B.3a recovery-unlock + 3 B.3b vault-creation + 3 B.4a folder-open + 4 B.4b read-block + 4 B.4c save-block + 4 B.4d share-block + 4 B.5 trash + restore); the Kotlin smoke runner covers 35 asserts (3 B.1.1.1 smoke + 5 B.2 password-unlock + 4 B.3a recovery-unlock + 3 B.3b vault-creation + 4 B.4a folder-open + 4 B.4b read-block + 4 B.4c save-block + 4 B.4d share-block + 4 B.5 trash + restore). The repository tracks every cryptographic decision in `docs/adr/` and pins every primitive against published KATs (NIST FIPS 203 / FIPS 204, RFC 8032 / 7748, RFC 5869, RFC 9106, BIP-39 Trezor canonical vectors). The CRDT merge layer is additionally proven commutative, associative, idempotent, and well-formed over the full record domain — including arbitrary tombstone-and-resurrection histories and arbitrary record-level `unknown` keys — by four `proptest` properties at default (~256 cases each) under `core/tests/proptest.rs`'s PR-C / PR #9 section. The bridge crate adds two round-trip proptests — `save_block` at 16 cases and `share_block` to N ∈ [1..4] recipients at 16 cases — and B.5 adds a third (`trash_restore_round_trip_preserves_block_fingerprint` at 16 cases) (all held low because per-case Argon2id-protected vault open dominates wall-clock time; raise once vault-open cost is amortizable across cases — issue #38).
