...
#
# CI replays the 87 committed inputs (no `corpus/` there). The 5.98 s test body
...
# #641 added 37 generated single-fault seeds for block_file and record; regenerate with
...
- **This DOES run in CI since #647, and the sentence it replaces is worth
  ...
  **State the residual scope exactly, because the wider claim is the one
  ...
  `core/fuzz/seeds/` plus `core/tests/data/diff_regressions/`, 87 inputs today (#641
  added 37 generated single-fault seeds for `block_file` and `record`).
