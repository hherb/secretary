...
| Manifest-shape assertions | Each write vector pins `expected.post_state.block_count` (post-op) and optionally `expected.post_state.find_block_uuid_hex` (a hex string asserts presence + uuid match). Share vectors add `expected.post_state.recipient_count`. **Absence** of a block after trash is covered implicitly by `block_count` (e.g. `trash_block_happy` pins block_count 2→1; the only way to reach block_count==1 is for `find_block(new_uuid)` to be `None`) — no separate "asserts is_none" pin is needed. | Minimum-information snapshot of "did the op observably succeed". Boolean-ish find_block + integer counts are stable across bindings without RNG dependency. |
...
- `find_block_uuid_hex` — optional. `"<hex>"` asserts `manifest.find_block(hex).is_some()` and hex-equals the returned summary's `block_uuid`. Absent / JSON null asserts nothing. Absence-after-trash is covered implicitly by `block_count` (see §2 manifest-shape assertions row).
...
2. **`find_block_uuid_hex`** — when present as a hex string, `manifest.find_block(uuid).is_some()` and the returned summary's `block_uuid` hex-equals the pin. When absent or JSON null, no find_block assertion runs (absence-after-trash is implied by `block_count`).
...
| Bridge regression: `trash_block` keeps the block findable after trashing | `trash_block_happy.expected.post_state.block_count` fails (would observe block_count=2 instead of the pinned 1; the same regression also breaks the downstream `restore_block_happy` chain because find_block returns the still-live entry) |
