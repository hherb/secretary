...
- No new `FfiVaultError` variant: `read_settings` / `write_settings` propagate the **existing** errors of `read_block` / `save_block`; bounds validation surfaces the **existing** `VaultError::InvalidArgument` at the binding wrapper (the bridge orchestrator trusts its caller for bounds — every caller, uniffi/pyo3 wrapper and desktop `io.rs`, invokes the shared pure `validate_save_settings` first; out-of-range clamps on next load anyway, so it is not a security surface). So the Swift/Kotlin `ConformanceErrors.*` harnesses and the core-KAT match are untouched.
...
- **uniffi** (`ffi/secretary-ffi-uniffi/`): add `read_settings` / `write_settings` to the `.udl` + the `namespace/mod.rs` fns; project the `Settings` dictionary (`wrappers/settings.rs`). **`SettingsWarning` is NOT projected** — mobile does not consume load warnings, and desktop reads them from the bridge `read_settings` return **directly** (not via the FFI); so the FFI surfaces `read_settings → Settings` only. The wrapper validates `device_uuid` length (→ `VaultError::InvalidArgument`, existing) and calls `validate_save_settings` before `write_settings` (→ `InvalidArgument` on out-of-range), per the "input validation at the binding wrapper" convention ([[project_secretary_input_validation_at_binding_wrapper]]). Exposes the bound constants (defaults/min/max) as namespace fns so the mobile UIs validate against one source.
