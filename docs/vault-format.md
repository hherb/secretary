...
| 2. definite-length encoding | **yes** (via the §4.3 step 4 re-encode, or a direct check — see the two-part requirement below) |
...
The requirement is behavioural rather than structural — no particular data
structure is mandated — but it has **two** parts, and a reader MUST meet both:
...
satisfies **(2)'s rules 2 and 3** inherently: a non-canonical length or integer
prefix no longer re-encodes to its own bytes, so §4.3 step 4 rejects it with no
...
What no reader may do is round-trip the subtree through a
...
**Resolved — conflict-copy trash-list merge-monotonicity (#401).** Earlier revisions of this document noted that the concurrent-write conflict-copy merge path did not reconcile trash lists at all, so a purged marker was not guaranteed to survive a manifest merge performed across a conflict copy. #401 closes that gap: the merge now reconciles `trash` by a block-level union keyed by `block_uuid`, takes the greatest `(tombstoned_at_ms, tombstoned_by, fingerprint)` triple per entry, and merges `purged_at_ms` monotonically (`Some` beats `None`, and among two `Some` values the greater timestamp wins) so a purge marker can never be lost or reverted by a merge — see crypto-design §11.6 for the full field-by-field rule and its commutativity/associativity/idempotence proof. A live-vs-trash collision left by the union (a `block_uuid` simultaneously live in `blocks` on one side and trashed on the other) is resolved purge-terminally: a purged trash entry always wins over a concurrent live/restored copy, otherwise the live copy wins. Had this gap remained open, it would have been a **durability** issue rather than a security hole: dropping a purged marker across that merge path at worst meant the purge did not stick for one device pairing — no plaintext or key was ever exposed to anyone who did not already hold a copy.
