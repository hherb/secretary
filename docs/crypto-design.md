...
5. **Duplicate map keys are forbidden** (RFC 8949 §5.4); decoders MUST reject input that contains them. This reject-MUST is scoped to the material a decoder *interprets* — every map whose keys it dispatches on. It does **not** reach inside a forward-compat unknown subtree (vault-format §6.3.2), whose contents the decoder cannot interpret and must reproduce byte-for-byte on re-encode; a decoder therefore accepts a repeated key, and a key order of its own, *within* such a subtree. Rule 1 is scoped the same way and for the same reason. This is a limit on what a decoder may reject, not a licence for an encoder: the version that authors an extension is bound by all six rules inside its own subtrees, and a decoder that understands the extension interprets it and applies them. vault-format §4.2 states the per-rule split and the preservation requirement in full.

Every such byte string is also bound by one limit that is not part of RFC 8949 §4.2.1's profile:

6. **Nesting depth is at most 256.** No chain of arrays, maps and tags, each held directly inside the one before it, may be longer than 256. The chain starts at the outermost item of the byte string being decoded, when that item is itself an array, map or tag. A scalar is not a level: an integer inside 256 nested arrays is within the limit, and a 257th array around it is not. Unlike rules 1 and 5, this limit is **not** scoped to interpreted material: it binds inside a forward-compat unknown subtree exactly as elsewhere, because a reader must walk a subtree's structure before it can retain it. Writers MUST NOT emit a longer chain, and readers MUST reject one. A tag is counted although rule 4 forbids tags, so that a body breaking both rules is reported the same way by every reader (vault-format §4.2's well-formedness precondition). This is a v1 profile bound, not a canonical-form rule. The reference implementation's readers have enforced exactly this limit since v1, as to which bodies they accept, so stating it narrows nothing a v1 reader accepts, and it bounds the stack a parser needs on input taken from the attacker-writable vault folder (§6 contact cards). Depth is counted from the root of the byte string being decoded, so a record inside a block plaintext is measured from the block's root.

A clean-room implementation passing the equivalent of `cbor2.dumps(record, canonical=True)` (Python) or sorting and shortest-form-encoding manually produces bit-identical bytes to this reference. That idiom is for **authoring your own bytes**; it is not a way to re-emit a forward-compat unknown subtree received from another version, because `canonical=True` re-sorts nested map keys and so destroys the entry order a reader is obliged to reproduce — an obligation that comes from vault-format §4.2's byte-preservation requirement and §4.3 step 4, not from rule 1's scoping, which merely permits such a subtree to arrive unsorted. The KAT files under `core/tests/data/` pin the byte form for v1 cards.
...
- The merged record carries the tombstoning side's identity metadata (`tags`, `record_type`), empty `fields` per §6.3, and `last_mod_ms = max(T_d, T_l)`. **Identity-metadata override**: this rule overrides §11.1's per-field LWW for `tags` and `record_type` *only when one side is tombstoned and the other is live* and the tombstoning side wins (the `LocalTombstoneWins` and `RemoteTombstoneWins` outcomes). It does not apply when both sides are tombstoned (§11.1 LWW applies normally) or when both sides are live (§11.1 LWW with the staleness filter applies). The override exists so that a UI surfacing a tombstoned record (in a trash bin, undelete prompt, or audit log) reflects the deleter's view of the record — its `record_type` and its `tags` at deletion time, not a concurrent edit they never saw. The override is safe (associativity-preserving) for `tags` and `record_type` because each is whole-vector LWW with greater `last_mod_ms` winning, and on a `*Wins` outcome the tombstoning side has `T_d ≥ T_l`, so the override agrees with §11.1 LWW; subsequent merges with same-`last_mod_ms` tombstones either fall into `BothTombstoned` set-union (`tags`) or `BothTombstoned` lex-larger LWW (`record_type`), both of which are themselves CRDTs.
- **Record-level `unknown` is excluded from the override.** §11.1 specifies a per-key lattice join (lex-larger canonical-CBOR bytes on collisions, plus single-side preservation) that does not depend on `last_mod_ms`. A wholesale-take of the tombstoning side's `unknown` map would not agree with this per-key join, and would break CRDT associativity: a tombstone in one merge order can swallow the live side's keys, while a different merge order ((live, live) first, then tombstone) preserves them. `unknown` is forward-compat metadata that v1 clients do not render, so the "deleter's-view" UX rationale is weaker than for `tags`/`record_type` — a v2 client that needs to capture identity metadata at deletion time MUST add an explicit v2 field with proper CRDT semantics (e.g., per-key timestamps) rather than relying on `unknown`.
