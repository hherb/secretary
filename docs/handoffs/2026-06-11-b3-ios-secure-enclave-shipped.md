...
**Key commits (squash collapses these):** spec `f38b183`, plan `f3c5afd`; T1 `c818dbc`/`da704a2`, T2 `bc77b48`/`8766185`, T3 `8b4dbf5`/`ab27217`, T4 `b8c541c`/`d903169`, T5 `d000957`/`9a1128e`/`1cba83e`, T6 `67476db`/`bf30a74`, T7 `9df0817`/`59b585c`/`cbce3fe`, T8 `4ecd26f`/`e074b78`, T9 `7109270`/`8d83cd0`, final-review runner reorder `1a502a9`; re-review fixups `facf676`/`751d542` (see below).

### Re-review fixups (PR #214, applied during the `/nextsession` resume — independent silent-failure pass)

A fresh-eyes re-review before merge (independent `silent-failure-hunter` pass over the SE conformer) surfaced two actionable items in `SecureEnclaveDeviceSecretStore.swift`; both fixed on-branch, rebuilt + full gauntlet re-run green:

- **`751d542` — Important: never mislabel a biometric auth failure as wrapped-secret tamper.** `mapDecryptError` only recognised auth failures in `LAError.errorDomain`, but `SecKeyCreateDecryptedData` on a biometry-gated SE key drives the `LAContext` implicitly, so user-cancel / auth-failed / unavailable outcomes frequently arrive in `NSOSStatusErrorDomain` — and those fell through to the final `.wrappedSecretCorrupt`. A benign **Cancel** would have read as "wrapped secret corrupt" (a tamper/data-loss signal that pushes a user to re-mint their slot). Fix: added an `NSOSStatusErrorDomain` auth-code branch **and flipped the fallback** so any unidentified failure → `.enclave(...)`, never `.wrappedSecretCorrupt`. `.wrappedSecretCorrupt` is now asserted only when the decrypt returned nil with no `CFError` at all (the one positively-identifiable corruption case). The safety property holds regardless of the exact on-device OSStatus taxonomy — that is **carried into the #202 manual proof: confirm which domain/code SE actually returns for cancel / no-match / lockout and for a genuinely corrupt blob.**
- **`facf676` — Minor: `clear()` attempts both Keychain deletes before throwing.** Previously a transient key-delete failure threw before the blob delete ran, stranding the blob (and since `isEnrolled` keys off the blob, the device kept reading as enrolled). Disenroll is a revocation primitive — attempt both, surface the first hard failure only after both ran.
