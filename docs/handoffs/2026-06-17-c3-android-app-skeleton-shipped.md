...
- **Post-PR `/fixall` (2026-06-17):** addressed four review observations on the open PR — (1) documented that a wrong-password silent pass intentionally routes to `SyncScreen` (the badge-tap re-prompt is the retry path, not a dead-end); (2) made `AppVaultProvisioning.stageGoldenVault` crash-safe (copy to a `.staging` sibling then atomic `rename`, so a crash mid-copy can't leave a half-written vault the `dest.exists()` short-circuit would later return); (3) the smoke test now resolves its state dir via the production `syncStateDir` helper; (4) bumped `activity-compose` 1.8.2 → 1.9.3 to align with the lifecycle 2.8.x line. Re-verified: host suites green, `:app:assembleDebugAndroidTest` resolves, 2/2 `MakeVaultSyncSmokeTest` green on `Medium_Phone_API_36.1`.
