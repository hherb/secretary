...
| **Typed error** (`:vault-access` `VaultBrowseError.kt`) | New `data class ReauthFailed(detail)` arm. `detail` is a curated, host-tested clause from the pure `reauthFailedMessage(DeviceUnlockError)` mapper (review fixup — was `e.toString()`), never a raw exception string/class name; integrity + generic arms fold to one message (no oracle, §13). Cross-module `when` audit found **no** exhaustive consumer `when (VaultBrowseError)` in production (`BrowseMapping.kt` maps *from* `VaultException` with `else`), so the arm broke nothing; full multi-module build confirms. |
...
| **`:app` wiring** | `unlockAndOpen` builds a **fresh per-unlock** `GraceWindowReauthGate(CoordinatorBiometricAuthorizer(coordinator, vaultId), clock = SystemClock::elapsedRealtime)` — a **monotonic** clock (review fixup): the grace window measures *elapsed* time, so a wall-clock source would let an NTP correction / user clock-set move the silent window; the `seed` in `openBrowseWithSync` shares that same time base. (iOS still uses wall-clock `Date` → parity follow-up #282.) `openBrowseWithSync(…, gate)` injects it into `VaultBrowseModel` (threaded into `RecordEditModel`) and **seeds** it at open (first post-unlock write is silent); reset on lock. All new gate params **defaulted** to `NoopReauthGate` → ~15 existing call sites/tests compile unchanged. |
