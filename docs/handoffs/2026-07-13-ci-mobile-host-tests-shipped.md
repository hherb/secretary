...
```
**Live CI (PR #425, run 29211079927):** both new jobs **green on the first run** — `android-host` 2m1s (**SDK auto-provisioned; no `sdkmanager` fallback needed**), `ios-host` 1m21s. 16 checks pass, 0 failing (the 2 pre-existing `cargo test` jobs were still completing at handoff, unaffected by this additive change).
...
- **#424 — CI hardening (filed this session, from PR #425's final review):** pin the Xcode/Swift toolchain on the macOS jobs (`swift-conformance` + the new `ios-host` both rely on `macos-latest`'s default Xcode) **and** add `timeout-minutes` uniformly across all `test.yml` jobs (none set it; a hung `swift test` burns the paid macOS runner to the 6h default). Pre-existing repo-wide convention; small, self-contained. **Acceptance:** macOS jobs pin the toolchain via a pinned-SHA action; every job sets `timeout-minutes`; applied uniformly.
...
- **Xcode/Swift toolchain unpinned + no `timeout-minutes`** — accepted here as **pre-existing repo-wide conventions** (the existing `swift-conformance` job is also unpinned; no job sets a timeout). Fixing only the new jobs would create stylistic drift; filed for uniform treatment as **#424**.
...
## Closing inventory
- **State on close:** PR #425 open on `feature/ci-mobile-host-tests` (worktree `.worktrees/ci-mobile-host-tests`). 4 branch commits (spec + plan + 2 task) + this handoff.
...
- **Acceptance:** iOS both packages green locally (46 + 276); Android `:vault-access:test` green locally (412). Live CI (PR #425, run 29211079927): `ios-host` + `android-host` both green on the **first** run (1m21s / 2m1s; Android SDK auto-provisioned, no fallback), 0 checks failing.
