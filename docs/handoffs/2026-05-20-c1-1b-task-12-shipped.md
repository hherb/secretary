...
- ~~**Test name slightly longer than the spec citation.**~~ **Resolved in PR #102 fixall pass:** design doc line 407 and plan §Task 9 prose now both cite the shipped name `commit_with_decisions_stale_manifest_hash_aborts_with_no_disk_writes`. The longer suffix advertises the second half of the assertion (byte-identical manifest post-abort), which the shorter form does not. `spec_test_name_freshness.py` continues to PASS (script's corpus is `crypto-design.md` / `vault-format.md` / `glossary.md` / ADRs only; the C.1.1b plan + design docs are out of its scope, so neither form ever entered the resolver count).
