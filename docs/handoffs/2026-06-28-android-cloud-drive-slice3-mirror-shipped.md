...
6. **Instrumented E2E + offline-flush + conflict-copy-ingest tests** (two working copies over one SAF tree, mirroring `cli/tests/two_instance_convergence.rs`). **Must exercise the two on-device-only factory paths the host suite can't reach: `findOrCreate` overwrite + the `delete()`-returns-false branch (a Slice-3 review fix that has never run against real SAF).** Also the deferred real-SAF `takePersistableUriPermission` round-trip from Slice 2.
