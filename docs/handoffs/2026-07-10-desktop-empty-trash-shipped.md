...
3. **#411** (destructive-trash post-op feedback) — surface the actual purge result ("Purged N items") from the `EmptyTrashReportDto` / `PurgeReportDto` / retention-report counts the DTOs already carry, closing the client-snapshot-count honesty gap consistently across `empty_trash` + `purge_block` + retention. UI-only (no `core`/`ffi`/bridge change).
...
5. **Manual GUI smoke (human-only, carried):** the #374 consent flow + the new retention/purge/empty-trash dialogs (`pnpm tauri dev` against a **temp copy** of a staged vault with old trash — [[feedback_smoke_test_temp_copy_golden_vault]]).
...
- **Report intentionally not surfaced**: `emptyTrash()`'s `EmptyTrashReportDto` return is discarded in the UI (empty list is the success signal; parity with per-block purge). The confirm text also asserts a *client-snapshot* count (`entries?.length`) rather than the actual on-disk purge count, so a concurrent multi-device sync mid-dialog could make the asserted count diverge from what is purged. Cross-cutting (applies equally to `purge_block` + retention), so **filed as #411** (surface "Purged N items" from the counts the DTOs already carry) rather than fixed only for `empty_trash` — out of this slice's FFI-wiring scope.
