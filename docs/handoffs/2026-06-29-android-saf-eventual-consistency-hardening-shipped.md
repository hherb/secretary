...
- **`write`** — bounded retry on `CloudFolderException` **plus a read-back byte-equality verify**. One attempt = `inner.write` → `inner.read` → `contentEquals`; a throw, an invisible read-back, or a wrong-bytes read-back retries the whole attempt (re-write is an idempotent overwrite). Absorbs all three eventual-consistency write failure modes; rethrows a typed `CloudFolderException` after `maxAttempts`. The read-back uses `inner.read` (not `this.read`) so it is a single probe per attempt — no budget-multiplying retry-within-retry.
