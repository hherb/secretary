...
- **CodeQL `rust/hard-coded-cryptographic-value` (alerts #130 → #131):** the `enroll_wrong_password_is_wrong_password_or_corrupt` test used a literal `b"wrong-password"` as the password arg, which CodeQL flags as a hard-coded cryptographic value. **First attempt** (a `[0u8; 24]` buffer filled by `OsRng.fill_bytes`) just moved the alert (→#131): CodeQL's dataflow latches onto the zero-init **constant** and doesn't model the in-place `fill_bytes` overwrite. **Fix that landed:** derive the wrong password from genuinely runtime data — `golden_password()` (a file read) with one byte XOR-flipped — so there is no constant for the rule to flag, and the result is guaranteed to differ from the real password. Lesson for [[feedback_test_crypto_random_not_hardcoded]]: against this rule, *deriving from a runtime source* beats *overwriting a literal buffer* — the latter leaves the constant in the dataflow. Test-only change; no spec/format/binding impact, conformance gauntlet unaffected.
