...
- **CodeQL `rust/hard-coded-cryptographic-value` (alerts #130 → #131 → #132):** the `enroll_wrong_password_is_wrong_password_or_corrupt` test used a literal `b"wrong-password"` as the password arg, which CodeQL flags as a hard-coded cryptographic value. It took **three iterations** to learn the rule's actual contract — it flags *any* constant that taints the value reaching the password sink, and CI re-runs the rule on each push: (1) `b"wrong-password"` literal → #130; (2) a `[0u8; 24]` buffer filled by `OsRng.fill_bytes` → #131 (dataflow latches onto the zero-init constant; the in-place `fill_bytes` overwrite is not modeled); (3) `golden_password()` with one byte XOR-flipped → #132 (the `0xff` mask is itself a constant in the dataflow). **Fix that landed:** `[golden_password(), golden_password()].concat()` — a value built *entirely* from runtime file reads with **no literal anywhere in the dataflow**; doubling guarantees it differs from the real single password, so the enroll is still rejected. Lesson for [[feedback_test_crypto_random_not_hardcoded]]: against this rule, the value reaching a crypto sink must derive *purely* from a runtime source — overwriting/masking a literal does NOT help, because the constant stays in the taint graph. Test-only change; no spec/format/binding impact, conformance gauntlet unaffected.
