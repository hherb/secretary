...
- **Post-review `/fixall` (2026-06-12):** two low-severity UX findings fixed in-branch — (a) `VaultSelectionViewModel.loadPersisted()` now preserves a surfaced `.unavailable` reason instead of silently downgrading it to `.located` on a screen re-appear (new regression test `testLoadPersistedPreservesUnavailableReason`); (b) demo-vault staging failure now surfaces in the screen's Error section (`onOpenDemo` made throwing) rather than leaving the button silently inert. A third finding (inaccessible-folder error conflation) was investigated and dismissed: the FFI maps an unreadable folder to the distinct `FolderInvalid` → `.folderInvalid`, never the credential-conflated `WrongPasswordOrCorrupt` (a folder that can't be read never reaches the credential-check stage). Gauntlet re-run green.
