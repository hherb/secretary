...
- **AEAD nonce per rewrite** — Task 14 exercises the rewrite path TWICE (v1 commit + v2 commit on the same vault). `OsRng` drives both, so each commit produces a fresh nonce. The test pins this with `assert_ne!(b_v1, b_v2)` at step 9.5 — same plaintext encrypted twice MUST yield different ciphertext, which a deterministic-nonce regression would break. (The earlier `assert_ne!(b_v0, b_v1)` at step 3 confirms the v1 commit changed the disk at all; the v1-vs-v2 compare is the actual nonce-freshness contract.) Today both pass — AEAD nonces are 24 bytes of OsRng output, collision probability negligible.
...
- **File size of `core/tests/sync_merge_crash.rs`** — 327 LOC (new this session). Comfortable margin.
