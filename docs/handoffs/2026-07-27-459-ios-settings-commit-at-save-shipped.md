...
- **#469** — CI never compiles the iOS app target (`ios/SecretaryApp`), so `SettingsScreen.swift` and friends are gated only by a local `build-app.sh`; the SwiftUI type-check ceiling (exit 65) is invisible to host tests. Filed off the #468 review. **Acceptance:** either CI fails on a deliberately-introduced compile error under `ios/SecretaryApp/Sources/`, or a recorded wontfix. Cheapest fix is one more `xcodebuild build` step in the already-path-gated, already-xcframework-warm `macos-host.yml`.
...
- **Next:** #459 on-device confirm (fold into the next device session, INSIDE the grace window) · **#469** (CI iOS-app compile gate) · #467 (fold-site privacy guard decision) · #417 (render tests) · #447 (Tauri biometric decision) · #443/#444 (Linux/Windows presence).
