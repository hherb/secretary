...
**Issues filed this slice (7):** [#666](https://github.com/hherb/secretary/issues/666)
...
references in shipped source). **#641 stays open** for `contact_card`,
`bundle_file` and `vault_toml`.
...
every input — **`record` 7,476 of 7,476, `block_file` 137 of 137** — and CI
makes a strict comparison on each of 41 committed single-fault seeds. §(1c).
...
| (this commit) | this baton and the `NEXT_SESSION.md` retarget |
...
| Target | Seeds | Tokens |
|---|---|---|
| `block_file` | **19** | `container_malformed` 9, `array_sort_order` 4, `repeated_array_value` 4, `unsupported_version` 2 |
| `record` | **22** | `wrong_type` 5, `malformed_cbor` 4, `non_canonical_unclassified` 4, `rule4_tag_or_float` 3, `duplicate_map_key` 3, `missing_field` 2, `integer_out_of_range` 1 |

- Four `block_file` seeds use **three-entry tables faulted only at the second
  adjacent pair** (fix wave, `3bafc2ad`): with two entries, a reader checking
  the first pair alone was conformant (measured: such a reader ACCEPTS all four).
- **Committed replay inputs 50 → 91.** `MIN_CORPUS_INPUTS`: `record` 3 → 25,
  `block_file` 1 → 20. Those floors include the accepting bases, so the strict
  comparisons CI makes are **22** on `record` and **19** on `block_file`. With
...
- Section RTS **check 5** holds two-fault bodies built in-section and never
  committed, because §6.1/§6.3 fix no order (#618's rule; #668): **7 `record`
  cases plus 1 `block_file` parity case** (`[high, low, low]` →
  `array_sort_order`). Six record rows each name the drift they catch; the
  seventh is a regression pin the pre-#641 order also passed. The record rows'
  Rust twins are `core/src/vault/record_order_tests.rs` (7 tests).

### (1e) Measured results

**CI shape** (fix wave, at `be6c9125`): 45 passed; the finish lines read
`record: 25 of 25 input(s) compared, 25 committed` and
`block_file: 20 of 20 input(s) compared, 20 committed`; no disagreement or
harness-failure line.

**Full local corpus** (at `1d172304`, the main checkout's runtime corpus
symlinked in, removed after):
...
| Full-corpus agreement | not measured by token | `record` 7,476/7,476, `block_file` 137/137 |
| Committed replay inputs (CI) | 50 | **91** |
...
run, and `git status --short` was empty after each. **All 22 rows
`RED_AS_EXPECTED`.** Each table was measured at a different commit, named above
it.
...
- **The walk is wired into `record::decode` only.** `decode_manifest` and block
  plaintext decode keep ciborium's three leniencies in both languages (#666).
...
- **CI replays committed inputs only** (91). The full-corpus agreement is a
...
- **The design spec is the approved design, not the as-built record.**
  ...
  - `core/src/vault/record_order_tests.rs` (Rust twins of check 5) is not in
    §9's file list;
  - Section WF holds **43** cases, not the plan's 35 (the walk branches added in
    `19bce3d3`);
  - **41** generated seeds, not the plan's 37: four three-entry `block_file`
    tables faulted at the second pair;
...
ls core/fuzz/seeds/block_file/*__*.bin | wc -l    # 19
ls core/fuzz/seeds/record/*__*.bin | wc -l        # 22
...
- **Mixed `Co-Authored-By` trailers**: 40 commits name Opus, 2 name Sonnet (the
...
- **CI evidence:** read the replay step's finish lines in the
  ...
  `record: 25 of 25` and `block_file: 20 of 20` must appear.
...
cargo test --release --locked -p secretary-core \
  --features differential-replay --test differential_replay      # 45 passed; record 25 of 25, block_file 20 of 20
...
uv run core/tests/python/conformance.py                              # 0 FAIL, WF 43/43, REG 32/32
