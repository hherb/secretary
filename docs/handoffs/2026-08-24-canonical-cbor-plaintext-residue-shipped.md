...
Housekeeping → brainstorm → spec → 9-task plan → **subagent-driven execution, fresh implementer per task, independent review after each, whole-branch review at the end** → full gate run. **All 9 tasks complete. 21 commits. 11 fix rounds.** (Plus the #560 review-fix commits on top.)
...
**False claims in the design spec I wrote, each caught by review, recorded as corrections in place rather than silently edited.** This is the session's main lesson.

**The count and the location were themselves wrong, and the #560 review corrected both.** This said "Three … each recorded as a struck-through correction in the spec". The spec actually carries **11 `~~` markers across 7 struck blocks covering 6 distinct false claims**, plus a 7th correction disclosed *without* strikethrough — so "three" understates the discipline actually exercised. And claim 1 below is **not in the design spec at all**: `grep -n "sort buffer"` on the spec returns nothing. Its only uncorrected appearance is in the *plan* (`docs/superpowers/plans/2026-08-23-547-canonical-cbor-plaintext-residue.md`, which has zero `~~` markers); the real correction lives in shipped code, at `core/src/vault/canonical/value.rs`'s module doc. Getting a self-congratulatory count wrong in the paragraph about getting counts wrong is the lesson restating itself.

1. **"The sort buffer is not secret-bearing"** — false. `record.fields` keys are user-authored field names inside an encrypted record, which CLAUDE.md's own #474 section calls "a decrypted CBOR field name". Fixed by **elimination**, not wiping: `(len, bytes)` is exactly the RFC 8949 §4.2.1 order for `&str` keys, verified before ruling by 184,041 pairwise comparisons across every CBOR head-length class and multi-byte UTF-8 — **zero mismatches**. (The #560 review independently reproduced this in Python with its own CBOR head encoder: 3,843-key corpus, 400,000 pairwise comparisons, 0 mismatches. **Neither sweep is committed as a test** — both live in prose. Filed as **#567**; what IS committed is a handful of chosen cases, and the golden-vault anchor cannot see a char-count regression at all, which is **#562**.)
...
**What is proven, stated precisely.** A wipe of freed heap is **not observable from safe Rust**, and neither is a reallocation `ciborium`'s parser performed before we saw the value. `Drop` panic-safety was traced and is clean — a panic in a `Drop` during unwind aborts the process, which would turn a leak into a crash. The wipe cannot be defeated by `mem::take`/`.clear()`/`.drain()` on `SecretValueTree`: its tuple field is module-private, it exposes no `&mut` and no consuming accessor, and — the stronger reason, which this sentence originally omitted — a type with a `Drop` impl cannot be destructured (E0509), so the guarantee survives even if the field's visibility widens.

**That sentence said "both tuple fields … neither type" until the #560 review, and for `SecretEntries` it was false.** That type DOES expose a consuming accessor — `take_next` — and its own doc comment says so outright, at length: a yielded `(key, value)` pair "leaves this type's protection entirely" and "NOTHING WIPES IT AUTOMATICALLY". The free function `wipe_leaked_value` exists precisely for that seam. Summarising the pair as "neither type" made the mechanism sound more absolute than it is, in the direction that hides the one real gap — and the #560 review then found that gap had in fact been fallen into at the only consumer (`bundle.rs` wiped a yielded value but dropped the yielded KEY intact, plus six `take_*` reject arms; all fixed in that review, one residual filed as #566).
...
- **State on close:** PR-ready, all 9 tasks done, full gate set green, whole-branch review returned **ready**. A further five-reviewer pass ran on the open PR (#560) and produced the fix commits above: the `bundle.rs` reject-arm residue, an effect test for `wipe_leaked_value` (whose wipe could be deleted with the whole suite green), the #557 block wipe test, the #559 visibility narrowing, an exact `ciborium` pin, three unreserved plaintext encode buffers, removal of a net-negative wrap in `manifest.rs`, two weak `> before` assertions tightened to exact counts, the container head-boundary sweep widened to 65536, and ten documentation claims corrected in place.
...
- **Filed by the #560 review:** **#561** (ciborium's 4 KiB `from_reader` scratch buffer holds decrypted plaintext, fixable via the public `from_reader_with_buffer`) · **#562** (the golden fixture is ASCII-only with no unknowns, so the frozen anchor is blind to a byte-vs-char comparator regression) · **#563**/**#564** (`block.rs` 2895 / `manifest.rs` 3844 lines) · **#565** (`re_encoded`) · **#566** (`set_once`'s duplicate-field temps) · **#567** (the key-order sweep is prose, not a proptest) · **#568** (`parse_manifest_map` has no duplicate-key check) · **#569** (three encode paths still copy secrets into an owned `Value`) · **#570** (ciborium's decode side reallocs from capacity 0 above 4 KiB).
- **Closed by the #560 review:** **#557** (block wipe test added, exact count) · **#559** (`pub` -> `pub(crate)` on the declarations).
