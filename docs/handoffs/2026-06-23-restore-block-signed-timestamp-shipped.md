...
- **Residual rollback vector (#293, follow-up):** suffix-equality closes the *larger-suffix-plant* and *authentic-file-removed* vectors, but **not** an in-place overwrite of the signed-suffix file with a genuinely owner-signed *older* copy (it is the only suffix match → selected; passes hybrid-verify → restored stale). Fully closing it needs a *content* commitment (hash / trashed `vector_clock_summary`) in the signed `TrashEntry`, verified post-decrypt — tracked in #293. So the "all six properties proven" / "undetectable rollback closed" framing applies to the suffix-selection surface only, not to content-freshness.
