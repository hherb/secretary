...
- **⚠️ PR #470 MUST be squash-merged.** `5c37a4f` is a knowingly-non-compiling commit (the deliberate break). The repo allows merge-commit and rebase merges too, and either would land that commit permanently on `main` — poisoning `git bisect` for exactly the failure class this slice exists to catch. Squash is the repo's convention (`0ecc5db`, `fee1622`), but the PR body now says so explicitly rather than assuming the right button gets pressed.
- **The PR contains one intentionally-red CI run.** Anyone scanning run history will see a red `macos-host` on this branch. That is the point — it is the evidence — but it needs the explanation to not read as a flake.
- **YAML swallowed the `#469` in a step `name:`.** `name: build-app.sh (iOS app compile proof, #469)` — an unquoted ` #` starts a comment, so the step rendered as `build-app.sh (iOS app compile proof,` with a dangling paren and no issue number. It is **valid YAML**, so `actionlint` stayed green and nothing flagged it; caught only by parsing the file and cross-checking the live `gh api .../jobs` step names. Quoted now, with a comment saying why. **Any `#` inside a workflow `name:` / `run:` string needs quoting** — and "actionlint is clean" is not evidence that a workflow string says what you meant.
...
- **`timeout-minutes: 45` is deliberately kept, not merely inherited.** The green run took 5m34s and the red one 4m29s — but both had a **warm** `rust-cache`, and the cold 4-triple cross-compile is precisely the case the cap exists to survive. The comment now records this instead of the original "re-tune once first-run duration is known"; revisit only once a genuinely cold run has been timed.
- **Only `macos-host.yml` gates the iOS app.** Review of PR #470 found one concrete out-of-gate break path and closed it: the root Rust manifests (`Cargo.toml` / `Cargo.lock` / `rust-toolchain.toml`) are now in the gate, because this is the only job that cross-compiles for the four Apple triples — `test.yml` runs on every PR but never cross-compiles, so a dep bump breaking `aarch64-apple-ios` used to reach `main` completely ungated. (The pre-fix baton claimed "no such path is known today"; that claim was wrong.) The gate is now `ios/** + ffi/** + core/** + the three root manifests + the workflow file`.
...
- **State on close:** PR **#470** on `feature/469-ci-ios-app-compile-gate` (worktree `.worktrees/469-ci-ios-app-compile-gate`), shipping **#469**. Net: **one workflow file changed** (+1 step, 3 `name:` edits, a widened path gate, header + rationale comments) plus a comment-only `run-ios-tests.sh` edit and docs. **No `core` / `ffi` / Swift / FFI-surface / on-disk-format change** — the final `git diff main... -- ios/SecretaryApp core/ ffi/` is empty.
- **Commits:** `29dc62d` (spec) · `8c388ea` (spec correction — bindings verified identical) · `ded4659` (plan) · `6d6bfc4` (**the workflow change**) · `5c37a4f` (deliberate break) · `cafd101` (revert) · `042d825` (ROADMAP + handoff). **Review fixups (PR #470 review):** `2b05456` (quote the step name — YAML ate `#469`) · `d7262b2` (stale "2 xcodebuild runs" comment) · `c73d23a` (gate on the root Rust manifests + ROADMAP) · `1275063` (two-caller drift note).
- **Acceptance:** `build-app.sh` **BUILD SUCCEEDED** · local mutation **exit 65** · `actionlint` **clean** · CI **red on exactly the new step** ([30723278616](https://github.com/hherb/secretary/actions/runs/30723278616)) → **green after revert** · product diff vs `main` **empty**. Fixups re-verified: `actionlint` clean · `yaml.safe_load` step names correct (`'build-app.sh (iOS app compile proof, #469)'`) · both path lists parse to the 7 expected globs · `shellcheck` unchanged (only the pre-existing SC1091 info on the `$SCRIPT_DIR` source line, identical on `HEAD`).
