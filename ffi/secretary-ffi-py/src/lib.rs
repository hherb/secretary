...
// B.4c save_block input types — re-imported under aliases so the bare
// `BlockInput` / `RecordInput` / `FieldInput` / `FieldInputValue` names
// can be the Python-facing pyclass names without colliding with the
// bridge crate's identical names.
use secretary_ffi_bridge::{
...
// ---------------------------------------------------------------------------
// B.4c: save_block input pyclasses + save_block pyfunction.
//
// The four input types are pure value carriers — they project the bridge
// crate's BlockInput / RecordInput / FieldInput / FieldInputValue 1:1.
// Python callers construct them with `BlockInput(block_uuid=..., ...)`
...
fn uuid_array_or_value_error(bytes: &[u8], field: &str) -> PyResult<[u8; 16]> {
