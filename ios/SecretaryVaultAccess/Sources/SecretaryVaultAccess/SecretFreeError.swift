...
extension VaultAccessError: SecretFreeError {
    ...
    /// is constructible in `ffi/secretary-ffi-bridge` only via a call into
    /// `error/detail.rs` (rules E2/E3/E4, CI-enforced) — a hand-rolled `format!`
    /// into that field now fails in the Rust author's own PR, so both halves of
    /// the string are structurally gated, not just the `core` half. Two residuals
    /// are documented guard LIMITS rather than silent gaps: an `io::Error` minted
    /// from a runtime string and folded through `core`'s `VaultError::Io` before
    /// reaching a gated field is not itself construction-site-checked (#487), and
    /// three syntactic re-wrap shapes (local binding plus shorthand, `detail:
    /// detail`, or a post-construction assignment) would need dataflow analysis
    /// to catch (#488). See `scripts/check-error-payload-hygiene.py` for the full
    /// rule set.
    ///
