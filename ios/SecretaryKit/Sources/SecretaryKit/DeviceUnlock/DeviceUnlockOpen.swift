...
/// vault's persisted settings). Pure Foundation/domain logic — no iOS-only UI API.
///
...
///
/// Hoisted from the iOS app target into SecretaryKit (D.5.2) so the iOS and macOS
/// apps share one copy of this security-sensitive flow (device-secret release +
/// zeroize).
